...
// import './apply.css'
import { Checkbox, message, Form, Input, Button, Upload, Modal } from 'antd';
import { LoadingOutlined, PlusOutlined } from '@ant-design/icons';
import { timer_clock, phone_reg } from '../config';
import { UPLOAD_IMG_URL } from '@/server/url';
import { _post_teachers_toogle } from '@/server/teachers';
...
const router = useRouter();
  ...
  const onFinish = async (values) => {
    ...
    const params = {
      tag: values.tag,
      certificates: imglist,
    };
    await _post_teachers_toogle(params).then((data) => {
        console.log('_post_teachers_toogle', data);
    ...
    const handleChange = (info) => {
        ...
        if (info.file.status === 'done') {
            ...
            if (info.file.response) {
                setFileList([
                  ...fileList,
                  { uid: info.file.uid, url: info.file.response.url },
                ]);
            }
        }
    };
            ...
            <div className="register_body">
              ...
              <div className="register_content">
                ...
                <div className="steps">
                  ...
                  {'   '}
                  <span
                    className={['step', step > 0 ? 'step_ok' : ''].join(' ')}
                  >
                    填写资料
                  </span>
                  {'   '}
                  <span
                    className={['step', step > 1 ? 'step_ok' : ''].join(' ')}
                  >
                    审核
                  </span>
                </div>
                ...
                {step === 0 ? (
                  ...
                  <div>
                    <div className="success_msg flex">
                      ...
                      <div>
                        <p className="success_one">资料提交成功！</p>{' '}
                ...
                <div className="next_btn" onClick={() => nextStep()}>
                  {step === 0 ? '下一步' : step === 1 ? '提交审核' : '返回首页'}
                </div>
              </div>
            </div>
          </div>
